...
exports.getTask = async (req, res) => {
  try {
    ...
    const task = await Task.findOne({ user: req.user.id, _id: String(req.params.taskId) });
    if (!task) {
...
exports.deleteTask = async (req, res) => {
  try {
    if (!validateObjectId(String(req.params.taskId))) {
      return res.status(400).json({ status: false, msg: "Task id not valid" });
    }

    let task = await Task.findById(String(req.params.taskId));
    if (!task) {
      return res.status(400).json({ status: false, msg: "Task with given id not found" });
    }
    ...
    await Task.findByIdAndDelete(String(req.params.taskId));
